// ============================================================
// src/browser/boss-ws/decoder.ts — Sprint 2026-07-13 PoC v3
// ============================================================
// BOSS WebSocket protobuf 解码（采纳 master 完整 7KB protoDefinition）。
//
// 关键修订（Sprint 2026-07-13 PoC v3）：
//   - 直接拷贝 ai-job-hunting-ui/src/webSocket/protobuf.ts:46 的 7KB protoDefinition
//   - 字段 id 与 boss 真实二进制完全对齐
//   - decode 单层（不再用 v2 的 repeated bytes 双层方案 —— 完整 schema 单层即可，
//     protobufjs 自身会 skip unknown field，无需双层兜底）
//
// type 含义（master schema 确认）：
//   TechwolfMessageBody.type = 1=文本 / 3=图片 / 9=职位 / 10=简历
//   文本字段在 TechwolfMessageBody.text (field 3)
// ============================================================

import * as protobuf from 'protobufjs'

// ============================================================
// master 完整 7KB protoDefinition（拷贝自 ai-job-master）
// ============================================================
...
/** 解码后的标准消息结构（对齐 master schema） */
export interface DecodedMessage {
  type: number             // body.type (1=文本/3=图片/9=职位/10=简历)
  fromUid: string
  fromName: string
  toUid: string
  toName: string
  mid: string
  time: number
  text?: string           // body.text（type=1 文本）
  jobDesc?: { title?: string; company?: string; salary?: string; jobId?: string }
  resume?: { user?: string; description?: string; city?: string; position?: string }
...
function ensureLoaded(): protobuf.Type {
  if (MessageType) return MessageType
  root = protobuf.parse(PROTO_DEFINITION).root
  MessageType = root.lookupType('TechwolfChatProtocol')
  return MessageType
}

/**
 * 解码 BOSS WebSocket 二进制协议（单层 decode，用 master 完整 schema）
 *
 * 协议栈：Boss 真实二进制格式严格遵循 master protoDefinition。
 *   完整 schema 已能覆盖所有字段；protobufjs 默认会 skip unknown field，
 *   不需要 v2 的"外层 bytes + 内层 try-catch"双层兜底。
 *
 * @param raw 来自 onmessage 的 e.data（Uint8Array）
 * @returns 解码后的消息数组
 */
export function decodeBossProtocol(raw: Uint8Array): DecodedMessage[] {
  const MessageType = ensureLoaded()
  // protobufjs 不接受 ArrayBuffer —— 转 Uint8Array
  const bytes = raw instanceof Uint8Array ? raw : new Uint8Array(raw as ArrayBuffer)
  const decoded = MessageType.decode(bytes) as unknown as {
    type: number
    messages?: Array<{
      from?: { uid?: string | number | { toString: () => string }; name?: string }
      to?: { uid?: string | number | { toString: () => string }; name?: string }
      mid?: string | number | { toString: () => string }
      time?: number | string | { toNumber: () => number; toString: () => string }
      body?: {
        type?: number
        text?: string
        jobDesc?: { title?: string; company?: string; salary?: string; jobId?: string | number }
        resume?: { user?: { uid?: string | number | { toString: () => string }; name?: string }; description?: string; city?: string; position?: string }
        image?: { originImage?: { url?: string; width?: number; height?: number }; tinyImage?: { url?: string } }
      }
    }>
  }
  const messages = decoded.messages ?? []
  return messages.map((m) => {
    const fromUid = m.from?.uid
    const fromUidStr =
      typeof fromUid === 'object' && fromUid !== null && 'toString' in fromUid
        ? (fromUid as { toString: () => string }).toString()
        : (fromUid as string | number | undefined)?.toString() ?? ''
    const toUid = m.to?.uid
    const toUidStr =
      typeof toUid === 'object' && toUid !== null && 'toString' in toUid
        ? (toUid as { toString: () => string }).toString()
        : (toUid as string | number | undefined)?.toString() ?? ''
    const midVal = m.mid
    const midStr =
      typeof midVal === 'object' && midVal !== null && 'toString' in midVal
        ? (midVal as { toString: () => string }).toString()
        : (midVal as string | number | undefined)?.toString() ?? ''
    const timeVal = m.time
    const timeMs =
      typeof timeVal === 'object' && timeVal !== null && 'toNumber' in timeVal
        ? (timeVal as { toNumber: () => number }).toNumber()
        : Number(timeVal) || 0
    const body = m.body
    return {
      type: body?.type ?? 0,
      ...
      jobDesc: body?.jobDesc
        ? {
            ...
            company: body.jobDesc.company,
            salary: body.jobDesc.salary,
            jobId: body.jobDesc.jobId?.toString(),
          }
        : undefined,
      resume: body?.resume
        ? {
            user: body.resume.user?.name ?? '',
            description: body.resume.description,
            city: body.resume.city,
            position: body.resume.position,
          }
        : undefined,
      image: body?.image?.originImage
        ? {
            url: body.image.originImage.url,
            width: body.image.originImage.width,
            height: body.image.originImage.height,
          }
        : undefined,
    }
  })
