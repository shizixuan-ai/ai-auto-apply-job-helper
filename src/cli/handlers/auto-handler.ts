// ============================================================
// src/cli/handlers/auto-handler.ts — Sprint C-2a §14.5 GREEN
// ------------------------------------------------------------
// 状态: GREEN (per §4.1, RED 已确认 import 失败)
// 覆盖: ADR §14.5 C-2a (R1 初始 login + R3 失败率监控 + R6 退出码精化)
// 纪律: §3.13 错误分层 (notifier msg 前缀 [AUTO.runner]);
//       §3.12 mock 注入 (fake clock + noop sleep + stub deps);
//       §3.10 refactor: throttle.ts 测试 caller 不影响 (mock 工厂保持兼容)
// 单账号红线守住: bossSearch / sendGreeting / loginByQR 全部 deps 注入, 默认 stub
// ============================================================
...
/** AutoHandlerDeps 注入 (per §14.2.3 关系图) */
...
export async function runDailyLoop(
  ...
  const stats: RunStats = { sent: 0, ok: 0, failed: 0, blocked: false }
